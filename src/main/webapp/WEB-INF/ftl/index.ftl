...
<html lang="en"><head>
    ...
    <script>
        $.fn.raty.defaults.path = "./resources/raty/lib/images"; // 指定raty组件的图片目录
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    for(var i = 0; i < list.length; i++) {
                        ...
                        var html = template("tpl", book); // 将数据结合下面的tpl模版生成html
                        $("#bookList").append(html);
                    }
                    $(".stars").raty({readOnly:true}); // 星型组件显示实际分数
        ...
        $(function() {
            loadMore(true); // 加载时调用loadMore加载第一页图书信息
            $("#btnMore").click(function() { // 加载更多图书
                /*
            var nextPage = $("#nextPage").val();
            $.ajax({
                url: "/books",
